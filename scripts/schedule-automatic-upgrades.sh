...
upgrade_match=`crontab -l | grep '/opt/talkyard.*/upgrade-if-needed'`
...
if [ -z "$upgrade_match" ]; then
	crontab -l | { cat; echo '10 4 * * * cd /opt/talkyard && ./scripts/upgrade-if-needed.sh >> talkyard-maint.log 2>&1'; } | crontab -
