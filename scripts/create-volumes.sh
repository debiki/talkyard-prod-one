#! /bin/bash

## The backups dir is special: it should be accessible to backup scripts.
#bkp_dir=/var/opt/backups/talkyard/
#mkdir -p $bkp_dir
#docker volume create --driver local --opt type=none --opt device=$bkp_dir --opt o=bind \
#          talkyard-backups

# See: https://docs.docker.com/reference/cli/docker/volume/create/

label_epoch="io.talkyard.epoch=1"

docker volume create --label "$label_epoch" talkyard-v1-pub-files
docker volume create --label "$label_epoch" talkyard-v1-priv-files
docker volume create --label "$label_epoch" talkyard-v1-cache-data
docker volume create --label "$label_epoch" talkyard-v1-pg-data
docker volume create --label "$label_epoch" talkyard-v1-es-data
docker volume create --label "$label_epoch" talkyard-v1-es-logs


